---
description: Generate a CLAUDE.md for the root of a bare git repo that orients Claude to the worktree layout
---
...
When Claude Code launches from a bare git repo root, it finds no source files and no project CLAUDE.md. This command generates a root-level CLAUDE.md that orients Claude: what this directory is, where the code lives, what can and can't be done from here, and where to find the real project instructions.

The generated file is untracked (bare repos have no working tree to commit from), so it won't conflict with any branch's CLAUDE.md.

## Contract

**Inputs:** None required. Discovers everything from git state.
**Outputs:** `CLAUDE.md` written to the current directory (the bare repo root).
...
4. **Check for a project CLAUDE.md:**
   Look for `CLAUDE.md` in the main worktree. If it exists, the generated file should reference it. If it doesn't, note that no project CLAUDE.md was found.
...
7. **Generate `CLAUDE.md`** with these sections:
   ...
   - **Critical rules** — Three rules that prevent the most common mistakes:
     ...
     2. The project CLAUDE.md is at `<main-worktree>/CLAUDE.md` — read it before doing project work.
...
8. **Write the file** to the current directory as `CLAUDE.md`. If one already exists, show the user a diff of what would change and ask before overwriting.
