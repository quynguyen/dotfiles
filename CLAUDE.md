...
- Two herdr setup steps are imperative and cannot be stowed: `herdr machine add quy@leopard --label leopard` (it contacts the remote and starts its server before saving, and machine profiles live outside `config.toml`), and the `Host leopard` block in `~/.ssh/config` (untracked on purpose - this repo is public and the ssh config would publish the tailnet name, hostnames and key filenames).
